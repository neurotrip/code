...
void add_psngr(struct ticket p_data[], char flight_name[]);
void free_place(struct ticket p_data[]);
void list_place(struct ticket p_data[]);
void delete_place(struct ticket p_data[], char flight_name[]);
void default_data_flight(char flight_name[]);
void open_data_flight(char flight_name[], struct ticket p_data[]);
...
int main(void)
{
    struct ticket p_data[NUM_TICKETS];
    char choice;
	int flight_number = 0;
	char flight_name[400];
	char flight_name_102[400] = "102_flight.data";
	char flight_name_311[400] = "311_flight.data";
	char flight_name_444[400] = "444_flight.data";
	char flight_name_519[400] = "519_flight.data";
	
	default_data_flight(flight_name_102);
	default_data_flight(flight_name_311);
	default_data_flight(flight_name_444);
	default_data_flight(flight_name_519);
	
	printf("Выберите нужный рейс 102, 311, 444, 519\n");
	scanf("%d", &flight_number);
	if (flight_number == 102) {
		strcpy(flight_name, flight_name_102);
		open_data_flight(flight_name_102, p_data);
	} else if (flight_number == 311) {
		strcpy(flight_name, flight_name_311);
		open_data_flight(flight_name_311, p_data);
	} else if (flight_number == 444) {
		strcpy(flight_name, flight_name_444);
		open_data_flight(flight_name_444, p_data);
	} else if (flight_number == 519) {
		strcpy(flight_name, flight_name_519);
		open_data_flight(flight_name_519, p_data);
	}
    ...
    while ((choice = getchar()) != 'f') {
        ...
        case 'c' :  show_sorted_occupied_seats(p_data);
                    break;
        case 'd' :  add_psngr(p_data, flight_name);
                    break;
        case 'e' :  delete_place(p_data, flight_name);
                    break;
        }
...
void default_data_flight(char flight_name[])
{
	FILE *fp;
	struct ticket p_data[NUM_TICKETS];
	fp = fopen(flight_name, "r");
    if (fp == NULL) {
        fp = fopen(flight_name, "a+");
        for (int i = 0; i < 12; i++) {
            p_data[i].place_number = i + 1;
            p_data[i].place_status = 0;
            strcpy(p_data[i].name_psngr, "n0ne");
