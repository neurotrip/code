#include <stdio.h>
#include <ctype.h>
#include <string.h>
#include <stdlib.h>

#define MAXWORD 100
#define BUFSIZE 100
#define MAXLINES 1000

char buf[BUFSIZE];
int bufp = 0;

struct list {
	int lines[MAXLINES];
	int count;
};

struct tnode {
    char *word;
    struct list *lines;
    struct tnode *left;
    struct tnode *right;
};

char *noiseWords[] = {"for", "or", "in", "and"};
int isNoiseWord(char *);
struct tnode *addtree(struct tnode *, char *, int lineNum);
void treeprint(struct tnode *);
int getword(char *, int, int *);
int getch(void);
void ungetch(int c);

int main(void)
{
    struct tnode *root;
    char word[MAXWORD];
	int lineNum = 0;
    root = NULL;
    while (getword(word, MAXWORD, &lineNum) != EOF) {
        if (isalpha(word[0]) && !isNoiseWord(word))
            root = addtree(root, word, lineNum);
	}
    treeprint(root);

    return 0;
}

int isNoiseWord(char *word)
{
	int n = sizeof(noiseWords) / sizeof(noiseWords[0]);
	for (int i = 0; i < n; i++) {
		if (strcmp(word, noiseWords[i]) == 0) {
			return 1;
		}
	}
	return 0;
}

struct tnode *addtree(struct tnode *p, char *w,int lineNum)
{
    int cond;

    if (p == NULL) {
        p = (struct tnode *)malloc(sizeof(struct tnode));
        (*p).word = strdup(w);
		(*p).lines = (struct list *)malloc(sizeof(struct list));
        (*p).lines->count = 1;
		(*p).lines->lines[0] = lineNum;
        (*p).left = (*p).right = NULL;
    } else if ((cond = strcmp(w, (*p).word)) == 0) {
        if (p->lines->count == 0 || p->lines->lines[p->lines->count - 1] != lineNum) {
			p->lines->lines[p->lines->count++] = lineNum;
		}
    } else if (cond < 0) {
        (*p).left = addtree((*p).left, w, lineNum);
    } else {
        (*p).right = addtree((*p).right, w, lineNum);
    }

    return p;
}

void treeprint(struct tnode *p)
{
    if (p != NULL) {
        treeprint((*p).left);
        printf("%s:", p->word);
		for (int i = 0; i < p->lines->count; i++) {
			printf(" %d", p->lines->lines[i]);
		}
		printf("\n");
        treeprint((*p).right);
    }
}

int getch(void) {
    return (bufp > 0) ? buf[--bufp] : getchar();
}

void ungetch(int c) {
    if (bufp >= BUFSIZE)
        printf("ungetch: слишком много символов\n");
    else
        buf[bufp++] = c;
}

int getword(char *word, int lim, int *lineNum) {
    int c;
    char *w = word;

    while (isspace(c = getch())) {
	}

    if (c != EOF)
        *w++ = c;

    if (!isalpha(c)) {
        *w = '\0';
        return c;
    }

    for (; --lim > 0; w++) {
		*w = getch();
		if (*w == '\n') {
			(*lineNum)++;
		}
        if (!isalnum(*w) && *w != '_') {
                ungetch(*w);
                break;
            }
        }
    

    *w = '\0';
    return word[0];
}

